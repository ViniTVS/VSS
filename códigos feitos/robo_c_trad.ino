...
//Verifica se está em freio elétrico.
bool is_motor_locked(void){   // retorna 1 se o motor estiver travado
  bool testeA, testeB, testeF;

  testeA = bitRead(motor.config.pwm_motor_A, 0) ^ bitRead(motor.config.pwm_motor_A, 1); // faço o xor dos bits de ambas as pontes
  testeB = bitRead(motor.config.pwm_motor_B, 0) ^ bitRead(motor.config.pwm_motor_B, 1); // já que 00 e 11 são freio elétrico
...
void read_message (TRadioMsg message) {
  ...
  switch(ID) {
    case 'M':
      pad  = message.conf.pad;
      messageM(pad, message.conf.data.data1,  message.conf.data.data2);
    break;

    case 'S':
      pad = message.conf.pad;
      messageS(pad, message.conf.data.data);
    break;

    case 'R':
      messageR(message.conf.data.data);
    break;
